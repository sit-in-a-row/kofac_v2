...
var selectedValue; // 전역 변수로 설정

slider.addEventListener("input", function() {
    selectedValue; // 슬라이더 값 업데이트
...
const setNoise = (weighted = false) => {
    for (let r = 0; r < rows; ++r) {
        for (let c = 0; c < cols; ++c) {
            if (cellsFromIframe[r][c] != 3 && cellsFromIframe[r][c] != 4 && Math.random() < 0.3) {
                if (weighted) {
                    cellsFromIframe[r][c] = 2;
                } else {
                    cellsFromIframe[r][c] = 1;
                }
            }
        }
    }
...
const removeWallBetween = (idx1, idx2) => {
    ...
    if (col1 > col2) {
        if (cellsFromIframe[row1][col1 - 1] != 3 && cellsFromIframe[row1][col1 - 1] != 4)
            cellsFromIframe[row1][col1 - 1] = 0;
    } else if (col1 < col2) {
        if (cellsFromIframe[row1][col2 - 1] != 3 && cellsFromIframe[row1][col2 - 1] != 4)
            cellsFromIframe[row1][col2 - 1] = 0;
    } else if (row1 > row2) {
        if (cellsFromIframe[row1 - 1][col1] != 3 && cellsFromIframe[row1 - 1][col1] != 4)
            cellsFromIframe[row1 - 1][col1] = 0;
    } else if (row1 < row2) {
        if (cellsFromIframe[row2 - 1][col1] != 3 && cellsFromIframe[row2 - 1][col1] != 4)
            cellsFromIframe[row2 - 1][col1] = 0;
    }
}

const animateMaze = (cellsFromIframe, idx) => {
    const animate = () => {
        if (!Array.isArray(cellsFromIframe) || cellsFromIframe.length === 0 || !Array.isArray(cellsFromIframe[0])) {
            console.error('Invalid or empty cells array');
            running = false;
            return;
        }

        if (idx >= cellsFromIframe.length * cellsFromIframe[0].length) {
            running = false;
            return;
        }
        ...
        if (cellsFromIframe[row] && cellsFromIframe[row][col] !== undefined) {
            if (cellsFromIframe[row][col] === 1) {
                $(`td[id=${row}-${col}]`).addClass('wall');
            } else if (cellsFromIframe[row][col] === 2) {
                $(`td[id=${row}-${col}]`).addClass('weight');
            }
        }
...
const gbfs = () => {
    ...
    const startCell = $('td.start, td.start.wall');
    const endCell = $('td.end, td.end.wall');    
    ...
    const startId = startCell.attr('id').split('-');
    const endId = endCell.attr('id').split('-');
...
const addNeighbor = (row, col, tmp, endIdx, currentIdx, visited, prev, endRow, endCol) => {
    if (row >= 0 && row < rows && col >= 0 && col < cols && !visited[get1DIdx(row, col)] && cellsFromIframe[row][col] != 1) {
...
const bfsHelper = (q, endIdx, prev, visited, distance = 0, iterationCount = 0) => {
    setTimeout(() => {
        ...
        for (const neighbor of neighbors) {
            const { r, c } = neighbor;
            if (r >= 0 && r < rows && c >= 0 && c < cols && cellsFromIframe[r][c] != 1 && !visited[get1DIdx(r, c)]) {
                const neighborIdx = get1DIdx(r, c);
...
var dfsHelper = (stack, endIdx, prev, visited, distance = 0, iterationCount = 0) => {
    setTimeout(() => {
        // 일시정지 상태 확인
        if (stopCondition) {
            // 일시정지 상태에서 재생 버튼이 클릭될 때까지 대기
...
function updateCell(cell) {
    ...
    if (classList.length == 0 || (!classList.includes('start') && !classList.includes('end'))) {
        ...
        cellsFromIframe[row][col] = cell.classList.contains('wall') ? 1 : 0;
    }
}
...
function handleMazeData(mazeInfo) {
    clearBoard();
    if (Array.isArray(mazeInfo) && mazeInfo.every(row => Array.isArray(row))) {
        return mazeInfo; // 정확한 배열 형태 확인
    } else {
        console.error('잘못된 데이터 형식:', mazeInfo);
        return []; // 형식이 맞지 않는 경우 빈 배열 반환
    }
}

// 각 iframe 내부에서 실행할 스크립트
window.addEventListener('message', function(event) {
    console.log(event);
    if (event.data.type === 'mazeInfo') {
        // mazeInfo 처리
        cellsFromIframe = handleMazeData(event.data.data);
        if (Array.isArray(cellsFromIframe) && cellsFromIframe.length > 0) {
            animateMaze(cellsFromIframe, 0);
            running = false;
        } else {
            console.error('잘못된 데이터 형식:', cellsFromIframe);
        }
    } else if (event.data.type === 'clearBoard') {
        console.log('보드 지우기 실행')
        clearBoard();
    } else if (event.data.type === 'clearPath') {
        console.log('경로 지우기 실행')
        clearPath();
    } else if (event.data.type === 'executeAlgorithm') {
        if (event.data.type === 'executeAlgorithm') {
    ...
    } else if (event.data.type === 'pauseBtn') {
        stopCondition = true;
    } else if (event.data.type === 'resumeBtn') {
        stopCondition = false;
    } else if (event.data.type === 'sliderValue') {
        selectedValue = event.data.data;
        console.log(selectedValue);
    } else {
        // 다른 데이터 유형 처리
        console.log('알 수 없는 메시지 유형:', event.data);
    }
}, false);